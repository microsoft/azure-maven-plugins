...
`<localDebugPort>` | false | Specifies the port for local debug, the default value is **5005**.
...
If you want to start the function host in debug mode, please add `-DenableDebug` as the argument. The function host use 5005 port by default, you may use another port by change the value of `localDebugPort`
```cmd
